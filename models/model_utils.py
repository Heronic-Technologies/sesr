...
from models.quantize_utils import (compute_ranges,
...
class LinearBlock_c(tf.keras.layers.Layer):
    def __init__(self,
        ...
        def conv2d(filters: int, kernel_size_: int, padding_: str) -> tf.keras.layers.Layer:
            return tf.keras.layers.Conv2D(filters=filters, kernel_size=kernel_size_, padding=padding_, dtype='float32')
    ...
    def build(self, input_shapes):
        ...
        delta = tf.eye(self.in_filters, dtype=tf.float32)
    ...
    def call(self, inputs):
        ...
        if self.mode == "train" or (self.collapsed_weights is None):
            ...
            # Keep delta in float32 for numerical stability during weight computation
            ...
            wt_tensor = tf.add(wt_tensor, self.residual)

            if self.mode == "infer":
                # store collapsed weights in the first inference, won't need to collapse again
        ...
        # Weights computed in float32 for stability, then cast to float16 if needed
        wt_tensor = tf.cast(wt_tensor, compute_dtype)
